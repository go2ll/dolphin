...
public class LeaveService {
	...
	public Leave getLeaveByWorkItemId(final long workItemId) throws Exception {
		...
		try {
			...
			String sql = "select * from DOLPHIN_EXAMPLE_LEAVE where PROCESS_INS_ID=" + wi.getProcessInstance().getId();
	...
	public void applyLeave(final long workItemId, final long applierId, final long dayNum, final String reason) throws Exception {
		...
		try {
			...
			pst = conn.prepareStatement("insert into DOLPHIN_EXAMPLE_LEAVE(ID,APPLIER,LEAVE_DAY,REASON,PROCESS_INS_ID,APPLY_DATE) values(?,?,?,?,?,?)");
	...
	@JTATransactional
	public void approveLeave(final long workItemId, final long userId, final boolean approveFlag) throws Exception {
		WorkItem wi = manager.getWorkItemById(workItemId);
		...
		try {
			conn = ds.getConnection();
			stmt = conn.createStatement();
			String sql = "update DOLPHIN_EXAMPLE_LEAVE set APPROVER=" + userId + ",APPROVE_FLAG='" + approveFlag + "' where PROCESS_INS_ID="
