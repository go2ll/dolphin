...
public class WorkflowManager {
	...
	public String getProcessEntryURL(String processDefId, long userId) {
		...
		if(firstManualActivities.size()==0) throw new IllegalStateException("process definition has no manual activity");
		if(firstManualActivities.size()==1) return firstManualActivities.get(0).getTask().getFormURL();
	...
	public long[] getProcessEntryPerformer(String processDefId) {
		ProcessDefinition processDef = getLatestVersionOfProcess(processDefId);
		List<Activity> firstManualActivities = processDef.getFirstManualActivities();
		if(firstManualActivities.size()==0) throw new IllegalStateException("no manual activity follows start activity");
		if(firstManualActivities.size()==1) return firstManualActivities.get(0).getTask().getAssignedPerformers();
		return null;
	}
	...
	ProcessDefinition getLatestVersionOfProcess(String processId) {
		List<ProcessDefinition> dolphinProcessList = dolphinProcessRepository.getProcessListByProcessId(processId);
	...
	ProcessDefinition getProcessDefByVersion(String processId, double version) {
		ProcessDefinition dolphinProcess = dolphinProcessRepository.getProcessByVersion(processId, version);
	...
	void saveProcessDefinition(ProcessDefinition def) throws DuplicateDefinitionException {
		if(getProcessDefByVersion(def.getProcessId(),def.getVersion())!=null) {
			throw new DuplicateDefinitionException("duplicate definition id="+def.getProcessId()+" version="+def.getVersion());			
		}
	...
	public WorkItem getUndoneWorkItem(long userId,long processInsId) {
		...
		if(results.size()>1) {
			...
			throw new DolphinException("the size of workitems for userid "+ userId+" on processId "+processInsId +" exceeds one! size="+results.size());
		}
	...
	public WorkItem getWorkItem(long workItemId,long userId) {
		...
		if(userId!=workItem.getUserId()) {
			throw new UnsupportedOperationException("Current user "+userId+" has no privilege to workitem@"+workItemId+"!");
		}
		return workItem;
	}
