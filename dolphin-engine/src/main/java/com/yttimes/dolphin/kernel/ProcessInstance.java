...
public class ProcessInstance implements Comparable<ProcessInstance> {
	...
	@Transient
	private Map<String, AssignmentHandler> dynamicAssignmentMap = new HashMap<String, AssignmentHandler>();
	...
	public boolean isUndoable() {
		if (getState() != State.SUSPENDED) {
			...
			List<String> preActiviesId = getProcessDefinition().getPreviousActivities(currentActivity.getActivityId());
	...
	public void start() {
		...
		ActivityInstance startNode = createActivityInstance(getProcessDefinition().getStartActivity().getId());
		startNode.start();
	...
	void undo() {
		...
		if (getState() == State.INITIALIZED || getState() == State.CANCELED || getState() == State.TERMINATED) {
	...
	void resume(String activityId) {
		...
		if (currentActivity == null) {
			...
			throw new IllegalArgumentException("cannot find activity in current process's activatedActivities,activityId=" + activityId);
		}
	...
	private ActivityInstance getActiveActivityInstance(String activityId) {
		...
		if (results.size() > 1)
			throw new DolphinException("cannot have more than one activity instance with same activity id");
		if (results.size() == 0)
			return null;
		return results.get(0);
	}
	...
	void cancel() {
		...
		if (getState() == State.RUNNING || getState() == State.INITIALIZED || getState() == State.SUSPENDED) {
	...
	private boolean isActivityReady(ActivityInstance activity) {
		if (activity.getState() == ActivityInstance.State.CANCELD || activity.getState() == ActivityInstance.State.TERMINATED) {
			return false;
		}
		if (activity instanceof StartActivity) {
			...
			List<String> activitiesId = getProcessDefinition().getPreviousActivities(activity.getActivityId());
			logger.debug("activity joinType:" + activity.getJoinType());
			logger.debug("getPreviousActivities of activity " + activity.getActivityId() + ":" + activitiesId);
	...
	void activateNextActivities(ActivityInstance activityIns) {
		if (undoMode) {
			...
			List<Transition> transitions = getProcessDefinition().getPreviousTransitions(activityIns.getActivityId());
			for (Transition transition : transitions) {
				...
				if (transition.validate(processDataFields)) {
					...
					if (activityInstance == null) {
						throw new IllegalStateException("cannot find an already executed activity instance,id=" + fromActivityId);
					}
					if (activityInstance instanceof ManualActivity) {
						removeActivityInstance(activityIns);
						activityInstance.undo();
					} else {
						throw new IllegalStateException("cannot undo process instance,preivious activity is not manual");
					}
				}
			}
			this.undoMode = false;
			...
			List<Transition> transitions = getProcessDefinition().getNextTransitions(activityIns.getActivityId());
			...
			if (nextActivities.size() == 0) {
				throw new DolphinException("cannot find next activity for " + activityIns.getActivityId() + "!processDataFields="
						+ processDataFields);
			} else if (!activityIns.isSplitedToNext() && nextActivities.size() > 1) {
				throw new ConfusedTransitionException("Too many transactions valid,size=" + nextActivities.size(), nextActivities);
			} else {
	...
	public List<Activity> getNextManualActivities(ActivityInstance activityInstance) {
		...
		List<Transition> transitions = getProcessDefinition().getNextTransitions(activityInstance.getActivityId());
		for (Transition transition : transitions) {
			String toActivityId = transition.toActivity();
			if (transition.validate(processDataFields)) {
