	...
	private void validateManualActivity(Activity activity) {
		...
		for (Task task : activity.getTasks()) {
			if (task.getType() != Task.Type.FORM) {
				throw new InvalidDefinitionException("Manual activity should has form task, activity id=" + activity.getId());
			}
		}
	...
	private void validateAutomaticActivity(Activity activity) {
		...
		for (Task task : activity.getTasks()) {
			if (task.getType() != Task.Type.APPLICATION) {
				throw new InvalidDefinitionException("Automatic activity should has application task, activity id=" + activity.getId());
			}
		}
	}
