 ...
 */
public class ProcessInstanceJPARepository extends JPARepository<ProcessInstance, Long>
  ...
  public List<ProcessInstance> queryAll() {
    return getEntityManager().createQuery("from ProcessInstance", ProcessInstance.class)
        .getResultList();
  }
  ...
  public List<ProcessInstance> queryByDefinitionId(long definitionId) {
    List<ProcessInstance> results = getEntityManager()
        .createQuery(
            "from ProcessInstance where processDef.id=:processDefinitionId order by updatedTime Desc",
  ...
  public List<ActivityInstance> getAllActivities(long processInsId) {
    List<ActivityInstance> activities = getEntityManager()
        .createQuery("from ActivityInstance  where processInst.id=:processInsId order by id",
