...
public class DolphinProcessJPARepository extends JPARepository<ProcessDefinition, Long>
    implements DolphinProcessRepository {
  ...
  public List<ProcessDefinition> getProcessListByProcessId(String processId) {
    List<ProcessDefinition> dolphinProcessList = getEntityManager().createQuery(
        "from ProcessDefinition where processId = :processId order by version desc",
        ProcessDefinition.class).setParameter(
  ...
  public ProcessDefinition getProcessByVersion(String processId, double version) {
    ...
    try {
      dolphinProcess = getEntityManager().createQuery(
          "from ProcessDefinition where processId = :processId and version = :version",
  ...
  public List<ProcessDefinition> getUsedProcessDefinitions() {
    List<ProcessDefinition> dolphinProcessList = getEntityManager().createQuery(
        "from ProcessDefinition d where d.id IN (select i.processDef from ProcessInstance i)",
        ProcessDefinition.class)
        .getResultList();
    return dolphinProcessList;
  }
  ...
  public List<ProcessDefinition> searchProcessDefinitions(String str) {
    List<ProcessDefinition> dolphinProcessList = getEntityManager().createQuery(
        "from ProcessDefinition where processId like '%" + str + "%'", ProcessDefinition.class)
        .getResultList();
    return dolphinProcessList;
  }
